...
def list_reports(
    category: Optional[str] = Query(
        None, description="Filter by category: patient | prediction | clinical | audit"
    ),
    report_status: Optional[str] = Query(
        None,
        alias="status",
        description="Filter by status: Ready | Generating | Completed | Archived | Failed",
    ),
...
def get_prediction_pdf(
    ...
    audit = (
        ...
        .filter(ClinicalPrediction.audit_uuid == prediction_uuid)
        .first()
    )
    if not audit:
        # Fallback query by string prefix or ID
        audit = (
            db.query(ClinicalPrediction).order_by(ClinicalPrediction.id.desc()).first()
        )

    if not audit:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Prediction audit record not found.",
        )

    raw_inputs = audit.inputs_json or {}
    recs_db = (
        db.query(Recommendation)
        .filter(Recommendation.prediction_audit_id == audit.id)
        .all()
    )
    recs_list = [
        {
        ...
        "top_positive_contributors": [
            {
                "feature": "Age",
                "impact": "+5.8%",
                "detail": f"Age: {raw_inputs.get('age', 60)} yrs",
            },
            {
                "feature": "Systolic BP",
                "impact": "+4.2%",
                "detail": f"BP: {raw_inputs.get('systolic_bp', 120)} mmHg",
            },
            {
                "feature": "Hypertension",
                "impact": "+3.5%",
                "detail": (
                    "Positive history" if raw_inputs.get("hypertension") else "Negative"
                ),
            },
        ],
        "top_negative_contributors": [
            {
                "feature": "Statin Therapy",
                "impact": "-2.8%",
                "detail": (
                    "Active prescription"
                    if raw_inputs.get("statin_history")
                    else "Naïve"
                ),
            },
            {
                "feature": "Heart Rate",
                "impact": "-1.4%",
                "detail": f"HR: {raw_inputs.get('heart_rate', 72)} bpm",
            },
        ],
        "patient_summary": {
            ...
            "risk_factors": (
                ...
                if raw_inputs.get("hypertension") or raw_inputs.get("diabetes")
...
def download_report_pdf(
    ...
    raw_inputs = (
        latest_audit.inputs_json
        if latest_audit
        else {"age": 60, "gender": 1, "systolic_bp": 120, "diastolic_bp": 80}
    )
    ...
    prediction_payload = {
        ...
        "recommendations": [
            {
                "category": "Lifestyle",
                "recommendation_text": "Implement low-sodium diet and daily physical activity.",
                "clinical_justification": "Primary Prevention",
            },
            {
                "category": "Medication",
                "recommendation_text": "Initiate statin therapy per ACC/AHA guidelines.",
                "clinical_justification": "Moderate Risk Threshold",
            },
        ],
        "top_positive_contributors": [
            {
...
def toggle_pin(
    ...
    if report.created_by != current_user.id:
        raise HTTPException(
            ...
            detail="You can only pin your own reports.",
        )
    ...
    db.refresh(report)
    logger.info(
        f"Report '{report.name}' pinned={report.pinned} by user {current_user.id}"
    )
    return report
...
def delete_report(
    ...
    if report.created_by != current_user.id and current_user.role.lower() not in [
        "admin"
    ]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
