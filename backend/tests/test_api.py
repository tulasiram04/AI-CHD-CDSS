...
from backend.database.models import Base, User, DoctorProfile, ModelRegistry, Patient, Admission, Diagnosis, LabResult
...
def test_direct_prediction_success():
    ...
    assert data["risk_level"] in ["Very Low", "Low", "Moderate", "High", "Very High", "Low Risk", "Borderline Risk", "Moderate Risk", "High Risk"]
...
def test_admission_prediction_success():
    ...
    if not adm:
        ...
        diag = db.query(Diagnosis).filter_by(admission_id=adm.id, icd_code="401.9").first()
        if not diag:
            diag = Diagnosis(
                admission_id=adm.id,
                icd_code="401.9",
...
def test_model_governance_flow():
    ...
    response = client.post(f"/api/v1/models/{model_id}/approve", json={"comment": "clinical OK"}, headers=doc_headers)
    assert response.status_code == 403
    
    # 3. Admin can approve model
    response = client.post(f"/api/v1/models/{model_id}/approve", json={"comment": "clinically validated"}, headers=admin_headers)
    assert response.status_code == 200
    data = response.json()
