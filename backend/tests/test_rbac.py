...
def setup_database():
    ...
    try:
        ...
        assigned = (
            ...
            .filter_by(user_id=seeded_users["lab tech"].id, patient_id=p1.id)
            .first()
        )
        if not assigned:
            db.add(
                PatientAssignment(user_id=seeded_users["lab tech"].id, patient_id=p1.id)
            )
            db.commit()
...
def test_patient_registration_rbac():
    ...
    # Succeeds for Nurse
    res = client.post(
        "/api/v1/patients", json=payload, headers=get_headers("rbac_nurse@hospital.org")
    )
...
def test_demographics_editing_rbac():
    ...
    res = client.put(
        ...
        json={"age": 46, "gender": 0},
        headers=get_headers("rbac_doctor@hospital.org"),
    )
    assert res.status_code == 200
    ...
    res = client.put(
        f"/api/v1/patients/{patient_id}",
        json={"age": 47},
        headers=get_headers("rbac_nurse@hospital.org"),
    )
    assert res.status_code == 200
    res = client.put(
        f"/api/v1/patients/{patient_id}",
...
def test_update_vitals_rbac():
    ...
    res = client.post(
        ...
        json={"systolic_bp": 130, "diastolic_bp": 85},
        headers=get_headers("rbac_nurse@hospital.org"),
    )
    assert res.status_code == 200
...
def test_upload_results_rbac():
    ...
    res = client.post(
        ...
        json={"lab_name": "Serum Glucose", "result_value": 110.0, "unit": "mg/dL"},
        headers=get_headers("rbac_labtech@hospital.org"),
    )
    assert res.status_code == 200
    res = client.post(
        f"/api/v1/patients/{patient_id}/labs",
        json={"lab_name": "Serum Glucose", "result_value": 110.0, "unit": "mg/dL"},
        headers=get_headers("rbac_ecgtech@hospital.org"),
    )
    assert res.status_code == 403

    # 2. ECG upload succeeds for ECG Tech, fails for Lab Tech
    res = client.post(
        f"/api/v1/patients/{patient_id}/ecg",
        json={"heart_rate": 75, "interpretation": "Sinus rhythm"},
        headers=get_headers("rbac_ecgtech@hospital.org"),
    )
    assert res.status_code == 200
    res = client.post(
    ...
    res = client.post(
        f"/api/v1/patients/{patient_id}/radiology",
        json={
            "modality": "CT",
            "exam_name": "Chest CT",
            "findings": "Normal",
            "impression": "No plaque",
        },
        headers=get_headers("rbac_radtech@hospital.org"),
    )
    assert res.status_code == 200
    res = client.post(
        f"/api/v1/patients/{patient_id}/radiology",
...
def test_predictions_rbac():
    ...
    # Doctor succeeds
    res = client.post(
        "/api/v1/predict", json=payload, headers=get_headers("rbac_doctor@hospital.org")
    )
    assert res.status_code == 200

    # Nurse fails
    res = client.post(
        "/api/v1/predict", json=payload, headers=get_headers("rbac_nurse@hospital.org")
    )
    assert res.status_code == 403
...
def test_governance_rbac():
    ...
    res = client.post(
        f"/api/v1/models/{model_id}/approve",
        json={"comment": "Approve this"},
        headers=get_headers("rbac_gov@hospital.org"),
    )
    assert res.status_code == 200
