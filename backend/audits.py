...
def list_prediction_audits(
    ...
    audits = (
        query.order_by(ClinicalPrediction.timestamp.desc())
        .offset(offset)
        .limit(limit)
        .all()
    )
...
def get_prediction_audit_detail(
    ...
    audit = (
        db.query(ClinicalPrediction).filter(ClinicalPrediction.id == audit_id).first()
    )
    if not audit:
        raise HTTPException(
...
def get_inference_performance_log(
    ...
    log = (
        ...
        .filter(InferenceLog.prediction_audit_id == audit_id)
        .first()
    )
    if not log:
        raise HTTPException(
