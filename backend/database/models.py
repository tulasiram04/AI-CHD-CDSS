...
class User(Base, AuditableMixin):
    ...
    role: Mapped[str] = mapped_column(String(50), default="doctor", nullable=False)  # admin, doctor, auditor, governance, nurse, etc.
...
class PendingRegistration(Base):
    ...
    email: Mapped[str] = mapped_column(String(255), unique=True, index=True, nullable=False)
    password_hash: Mapped[str] = mapped_column(String(255), nullable=False)
    role: Mapped[str] = mapped_column(String(50), nullable=False)  # Nurse, Lab Tech, etc.
    license_number: Mapped[str] = mapped_column(String(100), nullable=False)
    specialty: Mapped[str] = mapped_column(String(100), nullable=False)
    department: Mapped[str] = mapped_column(String(100), nullable=False)
    status: Mapped[str] = mapped_column(String(50), default="Pending", nullable=False)  # Pending, Approved, Rejected, Info Requested
...
class Patient(Base, AuditableMixin):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    patient_uuid: Mapped[str] = mapped_column(String(64), unique=True, index=True, nullable=False)  # Anonymized key
...
class Admission(Base, AuditableMixin):
    ...
    hadm_id: Mapped[int] = mapped_column(Integer, unique=True, index=True, nullable=False)
    ...
    statin_history: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, default=0)
    beta_blocker_history: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, default=0)
    ace_arb_history: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, default=0)
    aspirin_history: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, default=0)
    medication_count: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, default=0)

    __table_args__ = (
        Index("idx_admission_subject", "patient_id", "admittime"),
    )
...
class Diagnosis(Base, AuditableMixin):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    admission_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("admissions.id"), nullable=False)
...
class FeatureStore(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    feature_name: Mapped[str] = mapped_column(String(100), unique=True, index=True, nullable=False)
...
class ClinicalPrediction(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    audit_uuid: Mapped[str] = mapped_column(String(64), unique=True, index=True, nullable=False)
    clinician_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("users.id"), nullable=False)
    patient_uuid: Mapped[str] = mapped_column(String(64), index=True, nullable=False)  # PHI Separation
    ...
    risk_level: Mapped[str] = mapped_column(String(20), nullable=False)  # Low, Medium, High, Very High
    ...
    recommendations = relationship("Recommendation", back_populates="clinical_prediction")
    inference_log = relationship("InferenceLog", back_populates="clinical_prediction", uselist=False)

class PredictionHistory(Base):
    __tablename__ = "prediction_history"
    
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    prediction_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("clinical_predictions.id"), nullable=False)
    status: Mapped[str] = mapped_column(String(50), nullable=False)
...
class ModelRegistry(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    model_uuid: Mapped[str] = mapped_column(String(64), unique=True, index=True, nullable=False)
    version: Mapped[str] = mapped_column(String(50), nullable=False)
    ...
    status: Mapped[str] = mapped_column(String(50), default="Training", nullable=False)  # Training, Validation, Clinically Approved, Production, Retired
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
...
class InferenceLog(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    prediction_audit_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("clinical_predictions.id"), unique=True, nullable=False)
    ...
    clinical_prediction = relationship("ClinicalPrediction", back_populates="inference_log")

class ApprovalWorkflow(Base):
    __tablename__ = "approval_workflows"
    
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    model_artifact_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("model_registry.id"), nullable=False)
    step: Mapped[str] = mapped_column(String(50), nullable=False)  # Data Scientist, Clinical Reviewer, Governance Board
    ...
    signature_timestamp: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
...
class Notification(Base):
    ...
    sender_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, ForeignKey("users.id"), nullable=True)
...
class Appointment(Base, AuditableMixin):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    doctor_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("doctor_profiles.id"), nullable=False)
    patient_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("patients.id"), nullable=False)
...
class Recommendation(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    prediction_audit_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("clinical_predictions.id"), nullable=False)
    recommendation_text: Mapped[str] = mapped_column(Text, nullable=False)
    category: Mapped[str] = mapped_column(String(50), nullable=False)  # Lifestyle, Medication, Follow-up
    clinical_justification: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)

    clinical_prediction = relationship("ClinicalPrediction", back_populates="recommendations")

class Report(Base):
    __tablename__ = "reports"
    
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    name: Mapped[str] = mapped_column(String(255), nullable=False, default="Untitled Report")
    patient_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, ForeignKey("patients.id"), nullable=True)
    admission_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, ForeignKey("admissions.id"), nullable=True)
    prediction_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, ForeignKey("clinical_predictions.id"), nullable=True)
    report_type: Mapped[str] = mapped_column(String(50), nullable=False)
    category: Mapped[str] = mapped_column(String(50), nullable=False, default="clinical")
