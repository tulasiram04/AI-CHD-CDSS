...
def seed_database():
    ...
    try:
        ...
        # 2. Seed Single Primary Hospital
        # Clear extra mock hospitals if present
        db.query(Hospital).filter(Hospital.code != "SJH-01").delete(
        ...
        depts_data = [
            {
                "name": "Cardiology & CCU",
                "code": "CARD-01",
                "head_clinician": "Dr. Alexander Vance, MD",
                "status": "Active",
            },
            {
        ...
        for role_name, email in user_roles_emails.items():
            ...
            if role_name == "doctor":
                existing_profile = (
                    db.query(DoctorProfile).filter_by(user_id=user.id).first()
                )
        ...
        existing_model = (
            db.query(ModelRegistry).filter_by(model_version="v1.0.0").first()
        )
        ...
        for p_data in pending_samples:
            existing_p = (
                db.query(PendingRegistration).filter_by(email=p_data["email"]).first()
            )
            if not existing_p:
        ...
        if doctor_user and seeded_patients:
            ...
            for pred in sample_predictions:
                existing_pred = (
                    ...
                    .filter_by(patient_uuid=pred["patient_uuid"])
                    .first()
                )
            ...
            audit_events = [
                {
                    ...
                    "details": "Dr. Alexander Vance logged into Doctor Portal.",
                },
                {
                    "user_id": doctor_user.id,
                    "action": "CHD Prediction Executed",
                    "details": "Ran 10-year CHD risk calculation for patient.",
                },
            ]
        ...
        logger.info(
            "Successfully seeded PostgreSQL database with interconnected hospitals, departments, users, patients, pending registrations, and predictions."
        )
