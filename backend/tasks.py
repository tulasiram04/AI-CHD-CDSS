...
celery_app = Celery(
    "backend_tasks", broker=settings.REDIS_URL, backend=settings.REDIS_URL
)
...
def compute_daily_data_drift() -> dict:
    ...
    try:
        ...
        audits = (
            ...
            .filter(ClinicalPrediction.timestamp >= one_day_ago)
            .all()
        )

        if not audits:
            logger.info(
                "No prediction audits found in past 24 hours. Drift score calculations skipped."
            )
        ...
        logger.info(
            f"Analyzed {len(audits)} predictions in past 24 hours. Mean predicted risk: {mean_risk:.4f}"
        )
        return {
            "status": "success",
            "count": len(audits),
...
def generate_clinical_system_health_report() -> dict:
    ...
    try:
        ...
        logs = (
            db.query(InferenceLog).filter(InferenceLog.timestamp >= one_hour_ago).all()
        )
